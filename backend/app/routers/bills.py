...
def _extract_pdf_text(legis: LegiScan, texts: list[dict]) -> str | None:
    """Find the first PDF text doc, fetch it, and extract text with pdfplumber."""
    for t in texts:
...
def collect():
    """Collect bills from all PDF states, extract PDF text, and save to disk."""
    ...
    _save_bills(bills)
    log.info(f"Collection complete. {len(bills)} bills saved to {BILLS_FILE}")
